...
INPUT_CSV = "new--product_list copy 2.csv"
OUTPUT_CSV="new-ouput-11.csv"
FAILED_CSV = "new-_failed--12.csv"
...
#             retry_delay = random.randint(2500, 4500)
#             print(f"         🔁 Retry dalam {retry_delay} ms...")
#             page.wait_for_timeout(retry_delay)
...
def open_url_safely(page, url, max_retries=10):
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 1. Gunakan timeout yang lebih longgar dan wait_until 'domcontentloaded'
            # Ini lebih stabil untuk website yang berat
            response = page.goto(url, timeout=60000, wait_until="domcontentloaded")
            
            # 2. Cek status response untuk mendeteksi blokir
            if response and response.status in [403, 429]:
                print(f"        ⚠️ Terdeteksi Bot (Status {response.status}). Menunggu lebih lama...")
                page.wait_for_timeout(8000)
                continue

            # 3. KUNCI: Tunggu sampai elemen utama muncul sebelum mengambil data
            # Ini memastikan halaman tidak "blank" saat data dibaca
            page.wait_for_selector("h1[itemprop='name']", timeout=19000)
            
            # Kasih jeda sedikit agar JS selesai proses
            page.wait_for_timeout(1000)

            print(f"         ✅ Berhasil dibuka")
            return True
            ...
            # Jeda retry yang lebih manusiawi (acak 3-7 detik)
            retry_delay = random.randint(3000, 7000)
            print(f"         🔁 Retry dalam {retry_delay} ms...")
            page.wait_for_timeout(retry_delay)
    return False
...
def scrape_product(browser, url):
    # 4. Gunakan User-Agent asli agar tidak dicurigai sebagai bot 'headless'
    # Banyak situs memberikan halaman blank jika mendeteksi 'HeadlessChrome'
    ...
    # Note: Bagian blocking stylesheet dihilangkan karena sering menyebabkan halaman blank/tidak render sempurna
    page = context.new_page()
    ...
    try:
        ...
        # --- BAGIAN PENGAMBILAN DATA (TETAP SAMA SEPERTI KODE ASLI ANDA) ---
        h1 = page.query_selector("h1[itemprop='name']")
        brand_el = page.query_selector("h1[itemprop='name'] a[itemprop='brand']")
        ...
        price_el = page.query_selector('p[itemprop="price"]')
        price = price_el.inner_text().replace("Our price:", "").replace("$", "").strip() if price_el else ""
