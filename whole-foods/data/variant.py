...
# FIX: base harus sampai /dp/
BASE_URL = "https://www.amazon.com/dp/"

OUTPUT_CSV = "amazon_products-utama.csv"

# FIX: tambahkan 2 kolom yang dipakai dedupe/update
FIELDNAMES = [
...
def write_full_csv(rows_by_asin: dict, filename: str = OUTPUT_CSV):
    # FIX: ignore extra keys kalau ada
...
def parse_nutrition_table(soup) -> dict:
    """
    Return dict {label_lower: amount_text}
    """
    ...
    for tr in table.find_all("tr"):
        # label biasanya ada di span pertama, amount di span nutrientAmountText
        label_sp = tr.find("span")
        amount_sp = tr.find("span", class_=lambda c: c and "nutrientAmountText" in c)
        if label_sp and amount_sp:
            lab = label_sp.get_text(strip=True).lower()
            amt = amount_sp.get_text(strip=True)
            if lab and amt and lab not in out:
                out[lab] = amt
    return out

def get_nutrition_value(nutri: dict, label: str):
    return nutri.get(label.lower())
...
def get_soup(url, session):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "sorry, we just need to make sure you're not a robot" in low or "enter the characters you see below" in low:
...
def process_asin(target_asin, category=None):
    ...
    # FIX: URL benar
    soup, _ = get_soup(f"{BASE_URL}{target_asin}?th=1&psc=1", session)
    ...
    for i, v_asin in enumerate(variant_list, 1):
        ...
        # FIX: handle CAPTCHA pada varian juga
        if v_soup == "CAPTCHA":
            print("   CAPTCHA detected on variant:", v_asin)
            continue
        ...
        stars_el = v_soup.select_one("#acrPopover span.a-size-small")
        stars = stars_el.get_text(strip=True) if stars_el else None

        # OPTIMIZE: parse sekali, jangan berulang-ulang
        attrs = extract_amazon_attributes(v_soup)
        nutri = parse_nutrition_table(v_soup)
        ...
        # FIX: benar-benar simpan/update ke CSV
...
if __name__ == "__main__":
    ...
    # contoh run 1 ASIN
    process_asin("B07CX3GRXV", category="utama")
