...
for page in range(1, total_pages + 1):
    ...
    for prod_idx, product in enumerate(products, start=1):
        ...
        # Ekstrak data dasar dari list catalog jika nanti di detail produk kosong (Fallback)
        ...
        try:
            ...
            # --- SAFE FALLBACK INGREDIENTS ---
            # Sephora bisa menyimpan ingredients di 'ingredients', 'ingredientDesc', atau di 'currentSku'
            raw_ingredients = (
                product_detail.get("ingredients") or 
                product_detail.get("ingredientDesc") or 
                product_detail.get("currentSku", {}).get("ingredientDesc", "")
            )
            clean_ingredients = clean_html_text(raw_ingredients)

            # Daftar semua varian SKU produk
            ...
            for index, sku in enumerate(child_skus, start=1):
                time.sleep(random.uniform(0.5, 1.0))  # Delay per SKU untuk mengurangi risiko rate limit
                sku_id = sku.get("skuId", "")
                
                # --- SAFE FALLBACK VARIATION / SHADE ---
                variation_value = (
                ...
                variation_desc = sku.get("biography") or sku.get("variationDescription") or ""
                ...
                # Khusus jika ada SKU yang punya Ingredient tersendiri
                sku_raw_ing = sku.get("ingredientDesc")
                sku_clean_ing = clean_html_text(sku_raw_ing) if sku_raw_ing else clean_ingredients
                ...
                row = {
                    ...
                    "Product Ingredients": sku_clean_ing,
                ...
                print(f"   └─ [SKU {index}/{len(child_skus)}] ID: {sku_id} | Shade: {variation_value} | Price: {price}")
...
# Simpan ke CSV
csv_filename = f"{slug}_skus_fixed.csv"
csv_headers = [
