...
url = "https://w3dhihsi98-2.algolianet.com/1/indexes/*/queries"
...
# --- Ambil total produk dulu ---
payload = {
    "requests": [
        {
            "indexName": "prod_harrods.com",
            "query": "",
            "params": "facets=%5B%22*%22%5D&clickAnalytics=true&ruleContexts=%5B%22USD%22%5D&maxValuesPerFacet=1000&facetFilters=%5B%5B%22categoryPageIdentifier%3AMake%20Up%22%5D%5D&offset=0&length=60"
        }
    ]
}
...
print(f"Total produk: {nb_hits}, total halaman: {total_pages}")

# --- Loop semua halaman ---
for page in range(total_pages):
    offset = page * hits_per_page
    print(f"\n📥 Ambil data offset {offset}")
    ...
    page_data = resp.json()
    time.sleep(1)
    for item in page_data["results"][0]["hits"]:
        time.sleep(1)
