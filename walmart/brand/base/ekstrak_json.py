...
def simpan_data_ke_json(data, nama_file="single.json"):
    """Menyimpan dictionary ke file JSON"""
    try:
        with open(nama_file, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
        print(f"✅ Berhasil! Data disimpan di: {os.path.abspath(nama_file)}")
    except Exception as e:
...
if __name__ == "__main__":
    target_url = "https://www.walmart.com/ip/Great-Value-Parsley-Flakes-0-4-oz/861014617?classType=REGULAR"

    # Panggil fungsi yang sudah didefinisikan di atas
    data_hasil = ekstrak_json_next_data(target_url)

    if data_hasil:
        simpan_data_ke_json(data_hasil)
