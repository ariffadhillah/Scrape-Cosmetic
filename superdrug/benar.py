    ...
    while True:
        ...
        for attempt in range(2):
            try:
                ...
                page.wait_for_selector("div.product-grid__products-list", timeout=20000)
                break
...
def page_details(page, product_url, major_category=None):
    for attempt in range(2):
        try:
            ...
            page.goto(product_url, timeout=9000, wait_until="domcontentloaded")
            page.wait_for_selector("h1.product-details-title__text", timeout=5000, state="attached")
...
if __name__ == "__main__":
    with sync_playwright() as p:
        ...
        # daftar URL kategori
        category_urls = [
            "https://www.superdrug.com/skin/body-care/body-scrub/c/pt_skin_body_scrub",
            "https://www.superdrug.com/skin/body-care/body-oils/c/pt_skin_moisturising_body_oils"         
        ...
        for i, url in enumerate(all_product_links, 1):
            ...
            data = page_details(page, url, major_category="Fragrance")
            if data:
                results.append(data)
            time.sleep(random.uniform(1,2))  # delay kecil agar lebih stabil

        # simpan ke CSV
        filename = "body-tambahan.csv"
