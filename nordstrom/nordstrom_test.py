...
# =========================
# 1. API FETCHER (Mendapatkan Daftar URL)
# =========================
def get_all_product_urls():
    # URL API untuk kategori (contoh: Face Makeup)
    api_url = "https://www.nordstrom.com/api/browse/query/browse/beauty/makeup/face?top=72&postalCodeAvailability=23126&preferredStore=621&preferredPostalCode=23126&offset=9&page=2&isDynamicFacetsEnabled=true"
    ...
    'Referer': 'https://www.nordstrom.com/browse/beauty/makeup/face?offset=9&page=2&postalCodeAvailability=23126&preferredStore=621&preferredPostalCode=23126',
    'X-y8S6k3DB-b': 'h4vfmj',
    ...
    try:
        ...
        print(f"✅ Ditemukan {len(urls)} produk untuk diproses.")
        return urls
...
# =========================
# 3. MAIN RUNNER (SELENIUM)
# =========================
def run_scraper():
    # Ambil semua URL terlebih dahulu
    ...
    options.add_argument("--disable-blink-features=AutomationControlled")
    options.add_argument("--start-maximized")
    # options.add_argument("--headless") # Buka ini jika ingin berjalan di background
    ...
    try:
        # Kunjungi homepage dulu untuk dapatkan cookies dasar
        driver.get("https://www.nordstrom.com/")
        time.sleep(5)

        for index, url in enumerate(all_urls):
            print(f"🚀 [{index+1}/{len(all_urls)}] Memproses: {url}")
            driver.get(url)
            time.sleep(7) # Tunggu config muncul

            initial_config = driver.execute_script("return window.__INITIAL_CONFIG__ || null;")
            
            if initial_config:
                product_data = extract_skus_with_images(initial_config, url)
                all_data_to_save.extend(product_data)
                print(f"   ✅ Berhasil mengambil {len(product_data)} SKU")
            else:
                print(f"   ❌ Gagal: __INITIAL_CONFIG__ tidak ditemukan.")

        # Simpan semua ke CSV setelah selesai loop
        if all_data_to_save:
            filename = "testing-nordstrom_products-Face-Makeup.csv"
            keys = all_data_to_save[0].keys()
            with open(filename, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=keys)
                writer.writeheader()
                writer.writerows(all_data_to_save)
            print(f"\n✨ SELESAI! Data disimpan di {filename}")
