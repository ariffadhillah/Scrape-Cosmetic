...
def main():
    ...
    try:
        save_path = save_json(first_page, f"plp_page_1_off0")
        print(f"[SAVED] {save_path}")
    except Exception as e:
        print(f"[WARN] Gagal save JSON first page: {e}")
    ...
    for page_no in range(total_pages):
        ...
        try:
            save_path = save_json(plp_json, f"plp_page_{page_no + 1}_off{offset}")
            print(f"[SAVED] {save_path}")
        except Exception as e:
            print(f"[WARN] Gagal save PLP JSON offset={offset}: {e}")
    ...
    if products_data:
        ...
        combined_json = os.path.join(
            OUT_DIR,
            f"products_combined_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}.json"
        )
        with open(combined_json, "w", encoding="utf-8") as f:
            json.dump(products_data, f, ensure_ascii=False, indent=2)

        print(f"\n[SUCCESS] CSV saved: {CSV_FILENAME}")
        # print(f"[SUCCESS] JSON saved: {combined_json}")
