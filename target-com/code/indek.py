...
CATEGORY_CANDIDATES = (f"{key_category}", "testtest101")
PER_PAGE = 24
MAX_ATTEMPTS = 5
BACKOFF_BASE = 1.5
STORE_ID = 3991
...
def build_plp_params(offset, visitor_id, category):
    return {
        "category": category,
        "count": PER_PAGE,
...
def fetch_plp_variants(offset):
    ...
    for category in CATEGORY_CANDIDATES:
        visitor_id = random_visitor_id()
        params = build_plp_params(offset, visitor_id, category)

        try:
            data = request_json_with_rotation(
                base,
                params,
                label=f"PLP offset={offset} category={category}"
            )
            if data:
                return data
        except Exception as e:
            log.warning("PLP variant gagal offset=%s category=%s: %s", offset, category, e)
